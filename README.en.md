...
The Unity project itself is not distributed (models cannot be redistributed and are large).
On another machine you create a fresh project, drop this repository in, and carry the models by hand.

**This repository is all you need** — the physics engine is bundled, so there is no need to clone the
engine repository separately (do that only if you intend to modify the engine itself).

1. Create a new Unity project (URP template, Unity 6)
2. Install **UniGLTF** and **lilToon** (see "Requirements")
3. Clone this repository and copy the contents of `Assets/` into your project's `Assets/`

   ```bash
   git clone https://github.com/masaka1024/mmd2gltf-unity-physics-importer.git
   ```

4. Carry your model `.glb` files over by hand (USB / cloud / re-export from the original PMX)
5. Import the GLB, place it in the scene, then run **[1] physics → [2] materials → [3] hitboxes**

### What is not in git

| | Notes |
|---|---|
| Model `.glb` files | Not redistributable. Re-export from the original `.pmx` with `mmd2gltf-gui` if needed |
| Shared toons `toon01`–`toon10.bmp` | Ship with MMD |
| Scenes and converted materials | Regenerated by step 5 |
| Layers in ProjectSettings | The `MMDHitbox` layer is created automatically by step 3 |

### Sending changes back

Copy `Assets/Editor/` and `Assets/MMD_Scripts/` into this repository and commit. The Unity project is
not under version control, so this repository is the only sharing path.

**If you changed the engine (`Assets/MMD_Scripts/MmdPhysics/`), mirror the change into the source of
truth as well** (https://github.com/masaka1024/mmd2gltf-cs-physics).
