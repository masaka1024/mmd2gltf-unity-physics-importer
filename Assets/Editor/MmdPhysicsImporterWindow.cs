...
namespace Mmd2GltfImporter
{
    public class MmdPhysicsImporterWindow : EditorWindow
    {
        ...
        [SerializeField] private float tune_skirtColliderScale = 0.6f;    // スカートのコライダー縮小率
        [SerializeField] private float tune_hairColliderScale = 0.85f;    // 髪のコライダー縮小率
        [SerializeField] private float tune_maxDepenetrationVelocity = 1f; // めり込み解消速度の上限（衝突再有効化時の弾け防止）
        [SerializeField] private float tune_angularSlackDeg = 45f;        // 関節角度制限の遊び（度）
        ...
        private void OnGUI()
        {
            ...
            if (showTuningPanel)
            {
                ...
                GUILayout.Label(L("剛体の基本設定", "Rigidbody Basics"), EditorStyles.miniBoldLabel);
                ...
                GUILayout.Label(L("関節・ばね", "Joints & Springs"), EditorStyles.miniBoldLabel);
                ...
                GUILayout.Label(L("コライダー・衝突", "Colliders & Collision"), EditorStyles.miniBoldLabel);
                ...
                tune_maxDepenetrationVelocity = EditorGUILayout.Slider(L("めり込み解消速度の上限", "Max Depenetration Velocity"), tune_maxDepenetrationVelocity, 0.1f, 10f);

                GUILayout.Label(L("マテリアル", "Materials"), EditorStyles.miniBoldLabel);
            ...
            if (GUILayout.Button(L("1. 剛体とコライダーを配置 (古いエラーも自動掃除)", "1. Generate Rigidbodies & Colliders (auto-cleans old errors)")) && targetPrefab != null)
            {
                GenerateRigidBodies();
            }

            if (GUILayout.Button(L("2. ジョイントを結合 (必ず1の直後に実行)", "2. Connect Joints (run right after step 1)")) && targetPrefab != null)
            {
                ConnectJoints();
            }
