...
### アプリとしてビルドするとき（APK / exe）

**ビルドの前に必ず【1】物理を配線 / 再配線 を実行してください。** 配置しているモデルすべてに対して必要です。

【1】は GLB から物理に必要な部分（JSON チャンク = `extras.mmd` とボーン階層）だけを取り出し、
`Assets/MmdPhysicsData/<モデル名>.mmdphys.bytes` として保存して、`MmdPhysicsBehaviour` の
**`Physics Data`** 欄へ割り当てます。このアセットはビルドに同梱されるので、Android 端末でも読めます。
サイズは元 GLB の 1〜3%（実測 0.3〜1.7MB）です。中身は元 GLB のバイト列そのままなので、
エディタで `.glb` を直接読んだときと挙動は同一です。

> **2026-09-04 より前に配線したシーンは、必ず再配線してください。**
> 以前は `.glb` の絶対パス（`Glb Path`）だけを保存していました。Unity のビルドに `.glb` の
> **ソースファイルは含まれず**、エディタで解決した絶対パスも端末には存在しないため、
> **エディタでは動くのに APK では揺れ物がまったく動きません**（これが Android で物理が
> 反映されない症状の原因です）。`Physics Data` が空のままビルドした場合は、実行時に原因と
> 手順を LogError で出します。`adb logcat -s Unity` で確認できます。
...
> 「MMDと同等になった」という意味ではありません。数値での忠実度検証は モデルA 1 モデルでしか
