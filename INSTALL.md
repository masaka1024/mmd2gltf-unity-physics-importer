...
## `.glb` の取り込みは UniGLTF で

Unity 標準の glTF インポーター（glTFast）で取り込むと、**エラーも警告も出ないまま**髪やスカートが
体の正面へ回り込みます（取り込み時の軸変換が UniGLTF と違うため）。`.glb` を選択して Inspector の
見出しが `... (Glb Scripted Importer)` になっていることを確認してください。詳細と直し方は
[README](README.md#-glb-は必ず-unigltf-で取り込んでください) にあります。
